...
class BlockedEvent {
     ...
     * @param {FlowGraphNode} node
     */
    constructor(duration, stacks, node) {
    ...
    toString() {
        ...
        return `Slow synchronous block (${this.node.type}: ${Math.round(
...
class PerfAgent {
    ...
    _asyncInit(asyncId, type, triggerAsyncId) {
        ...
        // Prevent followsAsyncId and triggersAsyncId from being
        // cleaned up until this frame is also cleaned up.
        this._ref(asyncId, [followsAsyncId, triggerAsyncId]);
    }
    ...
    _asyncDestroy(asyncId) {
        ...
        const node = this._flowGraph.get(asyncId);

        if (node) {
            // Remove the references to followsAsyncId and triggersAsyncID
            // so that these might be cleaned up
            this._unref(asyncId, [node.followsAsyncId, node.triggerAsyncId]);
        }
    }
    ...
    _ref(asyncId, referencedAsyncIds) {
        for (const id of referencedAsyncIds) {
            if (!this._refs.has(id)) {
                this._refs.set(id, new Set());
            }

            this._refs.get(id).add(asyncId);
        }
    }

    _unref(asyncId, referencedAsyncIds) {
        for (const id of referencedAsyncIds) {
            const refs = this._refs.get(id);
    ...
    stop() {
        ...
        this._currentSkipList.clear();
        this._flowGraph.clear();
        this._refs.clear();
        this._timings.clear();
    }
}
