...
class Frame {
    ...
    constructor(callsite) {
        ...
        this.columnNumber = callsite.getColumnNumber();
    ...
    _formatFileLocation() {
        ...
        // Ignore eval and REPL origins for now
        return (
            (this.fileName ? this.fileName : '<anonymous>') +
            (typeof this.lineNumber === 'number' && this.lineNumber >= 0
                ? `:${this.lineNumber}`
                : '') +
            (typeof this.columnNumber === 'number' && this.columnNumber >= 0
                ? `:${this.columnNumber}`
                : '')
        );
    }
    ...
    _formatMethodCall() {
        if (this.typeName) {
            return `${
                this.typeName && !this.functionName.startsWith(this.typeName)
                    ? `${this.typeName}.`
                    : ''
            }${this.functionName}${
                // This test is simplified from: https://github.com/nodejs/node/blob/0f3c2c64d2fe73394e5a40c79f626d9d09c7cd5d/deps/v8/src/messages.cc#L575-L579
                this.methodName && this.methodName !== this.functionName
                    ? ` [as ${this.methodName}]`
                    : ''
            }`;
        }

        return `${this.typeName ? `${this.typeName}.` : ''}${this.methodName ||
            '<anonymous>'}`;
    }
    ...
    toString() {
        ...
        if (this.isMethodCall) {
            output += this._formatMethodCall();
        } else if (this.isConstructor) {
            output += `new ${this.functionName || '<anonymous>'}`;
        } else if (this.functionName) {
            output += this.functionName;
        } else {
            return output + this._formatFileLocation;
        }

        output += ` (${this._formatFileLocation()})`;
