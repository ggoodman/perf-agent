...
const ENABLE_DEBUG_MESSAGES = +process.env.DEBUG_PERF_AGENT;
...
class PerfAgent {
    ...
    _asyncInit(asyncId, type, triggerAsyncId) {
        ...
        const precursor =
            this._flowGraph.get(triggerAsyncId) ||
            this._flowGraph.get(followsAsyncId);
        const stack = this._useAsyncStackTraces ? createStack(2) : [];

        this._flowGraph.set(asyncId, {
            asyncId,
            followsAsyncId,
            stacks: precursor ? [...precursor.stacks, stack] : [stack],
            triggerAsyncId,
            type,
        });
    }
    ...
    _asyncAfter(asyncId) {
        ...
        if (ENABLE_DEBUG_MESSAGES) debug(`after(${asyncId})`);
    }
    ...
    _asyncDestroy(asyncId) {
        ...
        this._timings.delete(asyncId);
        if (ENABLE_DEBUG_MESSAGES) debug(`destroy(${asyncId})`);
    }
    ...
    _captureStackTraceForAsyncId(asyncId, receiver, callsite) {
        ...
        const node = this._flowGraph.get(asyncId);
        const stacks = node ? [...node.stacks, stack] : [stack];
...
function createStackRaw(callsite, stackDepth = Infinity) {
...
function createStack(skipFrames, callsite, stackDepth = Infinity) {
    const stack = createStackRaw(callsite, stackDepth);
...
function debug(str) {
    Fs.writeSync(
        2,
        `${str}: E: ${AsyncHooks.executionAsyncId()} T: ${AsyncHooks.triggerAsyncId()} \n`
    );
}
